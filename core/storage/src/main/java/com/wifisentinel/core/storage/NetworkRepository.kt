...
interface NetworkRepository {
    ...
    fun latestSnapshots(limit: Int = 100): Flow<List<NetworkSnapshot>>
    ...
    suspend fun clearHistoryForNetwork(networkIdHint: String)
}
...
class DefaultNetworkRepository(
    ...
    override fun latestSnapshots(limit: Int): Flow<List<NetworkSnapshot>> =
        snapshotDao.latestFlow(limit).map { entities -> entities.map { it.toDomain() } }
    ...
    override suspend fun saveFindings(findings: List<Finding>, timestampMs: Long) {
        ...
        val unique = findings.distinctBy { finding ->
            finding.dedupKey.ifBlank { finding.id }
        }
        unique.forEach { finding ->
            if (finding.dedupKey.isNotBlank()) {
                findingDao.deleteByDedupKey(finding.dedupKey)
            }
        }
        findingDao.insertAll(unique.map { it.toEntity(timestampMs) })
    }
    ...
    override suspend fun clearHistoryForNetwork(networkIdHint: String) {
        findingDao.deleteByNetworkId(networkIdHint)
        eventDao.deleteByNetworkId(networkIdHint)
        snapshotDao.deleteByNetworkId(networkIdHint)
    }
}
