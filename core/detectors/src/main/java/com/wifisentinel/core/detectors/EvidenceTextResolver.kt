...
object EvidenceTextResolver {
    fun label(context: Context, key: String): String {
        val resId = when (key) {
            ...
            EvidenceKeys.MAC_LOCAL_ADMIN_BIT -> R.string.evidence_mac_local_admin_bit
            EvidenceKeys.MAC_MULTICAST_BIT -> R.string.evidence_mac_multicast_bit
            EvidenceKeys.CURRENT_BSSID_OUI -> R.string.evidence_current_bssid_oui
            EvidenceKeys.EXPECTED_BSSID_OUIS -> R.string.evidence_expected_bssid_ouis
            EvidenceKeys.SAME_SSID_BSSID_COUNT -> R.string.evidence_same_ssid_bssid_count
            EvidenceKeys.BASELINE_RSSI -> R.string.evidence_baseline_rssi
            EvidenceKeys.CURRENT_RSSI -> R.string.evidence_current_rssi
            EvidenceKeys.DOWNGRADED_SECURITY_FOUND -> R.string.evidence_downgraded_security_found
            EvidenceKeys.OUI_CHANGES -> R.string.evidence_oui_changes
            EvidenceKeys.OBSERVED_OUIS -> R.string.evidence_observed_ouis
            EvidenceKeys.PORTAL_PHISHING_HINTS -> R.string.evidence_portal_phishing_hints
            EvidenceKeys.DNS_INTERCEPT_DOMAINS -> R.string.evidence_dns_intercept_domains
            EvidenceKeys.DNS_INTERCEPT_IP -> R.string.evidence_dns_intercept_ip
            else -> 0
        }
    ...
    private fun resolveLookalikeDiff(context: Context, rawValue: String): String {
        ...
        val labels = tokens.map { token ->
            when (token) {
                ...
                "invisible" -> context.getString(R.string.evidence_diff_invisible)
                "numeric" -> context.getString(R.string.evidence_diff_numeric)
